...
class AccountStandardLedger(models.TransientModel):
    ...
    reconciled = fields.Boolean(
        'Reconciled Entries', default=True,
    ...
    def _search_partner(self):
        if self.ledger_type == 'partner':
            if self.all_partners:
                return self.env['res.partner'].search([])
            elif not self.all_partners and not  self.partner_select_ids:
                raise UserError(_("Please select any partner"))
            else:
